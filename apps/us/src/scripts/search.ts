...
if (!modal || !modalInput || !results) {
  // Guard: element not in DOM (e.g. non-BaseLayout page)
  throw new Error('[search] Required DOM nodes missing')
}
...
interface HistoryEntry { title: string; url: string; ts: number }
...
function saveHistoryEntry(title: string, url: string) {
  if (!url) return
  title = (title || '').trim().slice(0, 120) || url
  const list = readHistory().filter((h) => h && h.url !== url)
  list.unshift({ title, url, ts: Date.now() })
  writeHistory(list.slice(0, HISTORY_LIMIT))
}
function removeHistoryEntry(url: string) {
  if (!url) return
  writeHistory(readHistory().filter((h) => h && h.url !== url))
}
...
function showHistory() {
  ...
  let out =
    ...
    '    <span class="lb-search-modal__history-label">Recent</span>' +
  ...
  list.forEach((h) => {
    out +=
      ...
      '  <a class="lb-search-modal__history-item" href="' + esc(h.url) + '" data-history-title="' + esc(h.title) + '">' +
      CLOCK_SVG +
      '    <span class="lb-search-modal__history-title">' + esc(h.title) + '</span>' +
      '  </a>' +
      '  <button type="button" class="lb-search-modal__history-remove" data-remove-history data-history-url="' + esc(h.url) + '" aria-label="Remove">' + REMOVE_SVG + '</button>' +
      '</li>'
  })
  out += '</ul></div>'
...
// Capture-phase delegated: save history + close on result/history link click
document.addEventListener(
  ...
  (e) => {
    ...
    if (rm && results!.contains(rm)) {
      removeHistoryEntry(rm.getAttribute('data-history-url') ?? '')
      showHistory()
      e.preventDefault()
      e.stopPropagation()
      return
    ...
    // Save history on result/history link click
    const a = target.closest('a[href]') as HTMLAnchorElement | null
    if (!a || !results!.contains(a)) return
    const title =
      a.getAttribute('data-history-title') ||
      (a.querySelector('.lb-search-modal__title') as HTMLElement | null)?.textContent ||
      a.textContent ||
      ''
    saveHistoryEntry(title, a.href)

...
const ORDER: Record<string, string[]> = {
  ...
  '开户与账户': ['如何下载长桥App', '登录注册', '开通香港综合账户', '开通新加坡综合账户', '香港与新加坡账户对比', '开户被拒怎么办', '模拟账户', '日内融账户', '虚拟资产账户', '专业投资者认证', '生物认证', '防诈骗提示', '账户常见问题', '账户切换', '我的', '我的费率', '我的卡券', '专享特权', '设置', '遗产处理', '标准账户停运与迁移'],

  // 入金（扁平结构）
  '入金': ['入金方式总览', '银行卡绑定与审核', '银行代码', '支持的银行与入金方式', 'FPS转数快', '网银转账入金', 'eDDA电子授权扣账', '银证转账', 'ATM与柜台支票', '信用卡入金', '电汇入金', '新加坡入金方式总览', 'PayNow即时转账', 'DDA自动扣款授权', '网银与电汇入金', 'Wise国际汇款', '标准账户入金指南', 'MT103汇款回执'],

  // 股票交易
  '股票交易': ['交易时间与规则', '订单类型', '交易费用', '分红与公司行动', '股票转仓', '桌面端工具'],
  '股票交易/交易时间与规则': ['港股交易规则与结算', '美股交易规则与结算', '新加坡股票交易规则与结算', '美股卖空', '美股定投'],
  '股票交易/订单类型': ['限价单与市价单', '碎股单', 'FOK与IOC订单', '附加订单', '到价反弹订单', '网格交易', '如何撤销订单'],
  '股票交易/交易费用': ['交易收费'],
  ...
  '行情数据': ['市场行情', '行情套餐', '行情订阅与权限', '行情数据字段介绍', 'ETF行情与全景', '个股F10资料', '个股财务分析', '个股公告与讨论', '资金流向', '发现', '动态', '搜索', '自选', '股单', 'LongbridgeAI信息助手'],


  // 衍生品（扁平结构）
  '衍生品': ['如何开通期权交易权限', '期权基础', '期权买方', '期权卖方', '期权策略', '期权延长时段交易', '期权行情字段介绍', '期权链', '期权计算器', '窝轮', '牛熊证'],

  // 融资融券（扁平结构）
  '融资融券': ['融资额度与保证金规则', '货币保证金', '追缴保证金规则', '融资利息规则', '账户风控状态', '如何关闭或降低融资额度', '融资常见问题'],

  // 基金与理财（扁平结构）
  '基金与理财': ['香港余额通', '新加坡余额通', '余额通交易规则', '余额通常见问题', '余额通', '基金基础与类型', '基金买入与卖出规则', '基金定投', '基金收费规则', '货币基金交易规则', '基金常见问题', '闲钱理财', '结构化产品'],

  // 新股认购
  '新股认购': ['新股申购', '暗盘交易'],

  // 出金（扁平结构）
  '出金': ['香港网银出金', '如何出金到香港银行卡', '电汇出金', '新加坡网银出金', '标准账户出金指南', '资金退款'],

  // 资产与账单（扁平结构，R3 合并 App功能/资产与持仓/）
  '资产与账单': ['资产概览', '资产设置', '全部功能', '现金', '基金', '财富', '存入资金', '加密货币', '盈亏页面释义', '持仓成本与盈亏计算', '盈亏分析操作指南', '结单操作与字段解释'],

  // 资金划转与换汇（扁平结构）
  '资金划转与换汇': ['主子账户资金划转', '跨账号划转', '香港货币兑换', '新加坡货币兑换', '自动还款'],
  ...
  // 故障排查
  '故障排查': ['APP网络连接排查', '收不到短信验证码', '恒生银行可疑账号澄清', '入金未到账怎么办', '出金失败怎么办', 'App登录失败怎么办', 'DDA授权失败怎么办', '订单提交失败怎么办', '期权行权异常怎么办', '联系客服'],
}
